...
// Conditioning HR targets below are absolute bpm computed for this athlete
// (age 44) using the KARVONEN method (%heart-rate reserve), not plain %HRmax.
//
// HRmax basis:
...
//
// RHR basis: validated resting HR from soccer_biometrics.rhr_bpm = 60.
// Reserve = 185 − 60 = 125.  target(bpm) = 60 + %HRR · 125.
//
// Karvonen zone table (%HRR → bpm), used by lib/hrZones.js zonesFor():
//   60% =135   70% =148   80% =160   90% =173   100% =185
//   Z1 ≤135 (recovery) · Z2 136-148 (aerobic) · Z3 149-160 (tempo)
//   Z4 161-173 (threshold) · Z5 174-185 (VO2max)
export const WORKING_HRMAX = 185;

// Validated resting HR (soccer_biometrics.rhr_bpm). Anchors the Karvonen
// reserve. Update if a new sustained resting baseline is recorded.
export const WORKING_RESTHR = 60;
      ...
      protocols: [
        {
          ...
          name: 'Norwegian 4×4',
          desc: '4 rounds: 4 min @ 167-176 bpm / 3 min recovery. Gold-standard VO2max protocol — the engine upgrade that lets you sustain second-half output. Treadmill works fine. Same speed all 4 intervals — if rep 4 felt easier than rep 1, push harder next time.',
          rpe: 9, tags: ['cond'], kind: 'norwegian_4x4',
          hr_low: 167, hr_high: 176, // 86-93% HRR (90-95% HRmax) — Z4/Z5, true VO2max
        ...
        },
        {
          ...
          desc: '20-30 min @ 130-145 bpm. Aerobic base — improves recovery capacity between sprints and total session volume tolerance. Nasal-breathing pace if possible.',
          rpe: 5, tags: ['cond'], kind: 'single_metric',
          hr_low: 130, hr_high: 145, // 56-68% HRR — true Z2 aerobic base (was mislabeled as 140-150 = threshold)
      ...
      protocols: [
        {
          ...
          desc: '4 rounds: 4 min @ 160-170 bpm / 3 min recovery. Same protocol as full, but cap intent at RPE 7.5. Still effective VO2max work, less CNS cost.',
          rpe: 7.5, tags: ['cond'], kind: 'norwegian_4x4',
          hr_low: 160, hr_high: 170, // 80-88% HRR — Z4 capped
        },
        {
          exercise_key: 'assault_bike_tabata',
...
export const MODE_INSIGHTS = {
  ...
  mod2: {
    ...
    cond: 'Zone 2 aerobic work only. 20 min max. HR ~125-145 bpm. This is an aerobic flush — supports recovery rather than creating new training stress. No sprint intervals at this readiness level.',
  },
  mod3: {
    ...
    cond: 'Active recovery movement only. Easy bike at RPE 3-4 or a walk. The goal is circulation and tissue recovery, not a training effect. Keep HR under ~125 bpm.',
  },
  recovery: {
    ...
    cond: 'Easy bike Zone 2 only. 15 min. No intervals, no sprints — keep HR under ~120 bpm.',
  },
};
